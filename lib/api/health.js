...
export default async function handler(req, res) {
  ...
  // Check database connection - DISABLED as Notion is primary data storage
  /*
  let pgClient;
  try {
  ...
  */
  ...
  // Default database status to ok if not explicitly checked
  if (health.checks.database.status === 'checking') {
    health.checks.database = { 
      status: 'ok', 
      message: 'Database check temporarily disabled, Notion is primary data storage' 
    };
  }
