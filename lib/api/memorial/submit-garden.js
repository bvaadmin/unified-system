...
import { sendMemorialConfirmation } from '../../email/sendgrid.js';
...
export default async function handler(req, res) {
  ...
  const DATABASE_URL = process.env.DATABASE_URL; // Vercel only has DATABASE_URL set
  ...
  try {
    ...
    // Extract personal history from JSON (only for immediate placements)
    let personalHistory = {};
    const isImmediatePlacement = properties['Application Type'] === 'immediate';
    
    if (isImmediatePlacement) {
      try {
    ...
    if (pgClient) {
      try {
        console.log('PostgreSQL client created, attempting connection...');
        console.log('Application Type:', properties['Application Type']);
        console.log('Is Simplified:', properties['Simplified']);
        await pgClient.connect();
        console.log('PostgreSQL connected successfully');
        
        // Handle all future/prepayment submissions
        if (properties['Application Type'] === 'future') {
          console.log('Processing prepayment submission');
          
          // Insert into prepayments table
          ...
          const prepaymentValues = [
            ...
            'active',
            `Prepayment for: ${properties['Prepayment Names'] || 'future placement'}`
          ];
          
          pgResult = await pgClient.query(prepaymentQuery, prepaymentValues);
          pgId = pgResult.rows[0].id;
          console.log('Prepayment record created:', pgResult.rows[0]);
        } else {
        ...
        // Call the database function to create person records and establish relationships
        const processResult = await pgClient.query(
          'SELECT bayview.process_memorial_submission($1)',
          [pgId]
        );
        console.log('Person records created and linked for memorial ID:', pgId);
        
        // Get the created person IDs for response
        const personQuery = await pgClient.query(`
          SELECT 
            m.contact_person_id,
            array_agg(mdp.person_id ORDER BY mdp.placement_order) as deceased_person_ids
          FROM bayview.memorials m
          LEFT JOIN bayview.memorial_deceased_persons mdp ON m.id = mdp.memorial_id
          WHERE m.id = $1
          GROUP BY m.contact_person_id
        `, [pgId]);
        
        if (personQuery.rows[0]) {
          personsCreated.push({
            contact_person_id: personQuery.rows[0].contact_person_id,
            deceased_person_ids: personQuery.rows[0].deceased_person_ids || []
          });
        }
        }
      ...
      try {
        ...
        if (properties['Application Type'] === 'future') {
          await updateClient.query(
            'UPDATE bayview.prepayments SET notion_id = $1 WHERE id = $2',
    ...
    // Send confirmation email if we have contact email
    let emailSent = false;
    if (properties['Contact Email']) {
      try {
        // Collect person names for the email
        const personNames = [];
        if (properties['Application Type'] === 'immediate') {
          if (properties['Deceased Name']) {
            personNames.push(properties['Deceased Name']);
          }
          // Add other persons if present
          if (properties['Other Person First Name'] && properties['Other Person Last Name']) {
            personNames.push(`${properties['Other Person First Name']} ${properties['Other Person Last Name']}`);
          }
          if (properties['Second Person First Name'] && properties['Second Person Last Name']) {
            personNames.push(`${properties['Second Person First Name']} ${properties['Second Person Last Name']}`);
          }
        } else if (properties['Application Type'] === 'future') {
          if (properties['Prepayment Person 1']) {
            personNames.push(properties['Prepayment Person 1']);
          }
          if (properties['Prepayment Person 2']) {
            personNames.push(properties['Prepayment Person 2']);
          }
        }
        
        emailSent = await sendMemorialConfirmation({
          contactEmail: properties['Contact Email'],
          contactName: properties['Contact Name'] || 'Applicant',
          submissionId: properties['Submission ID'],
          applicationType: properties['Application Type'],
          feeAmount: parseFloat(properties['Fee Amount']) || 0,
          isMember: properties['Bay View Member'] === 'Yes',
          persons: personNames
        });
        
        if (emailSent) {
          console.log('Confirmation email sent to:', properties['Contact Email']);
        }
      } catch (emailError) {
    ...
    return res.status(200).json({
      ...
      pgId: pgId,
      personsCreated: personsCreated.length > 0 ? personsCreated[0] : null,
      ...
      emailSent: emailSent,
      debug: {
        hadDatabaseUrl: !!DATABASE_URL,
        hadPgClient: !!pgClient,
        applicationType: properties['Application Type'],
        isSimplified: properties['Simplified']
      }
    });
