 ...
 * @param {string} data.formType - 'wedding', 'memorial-funeral-service', or 'baptism'
...
export async function sendChapelConfirmation(data) {
  ...
  const serviceLabel = isWedding ? 'Wedding' : isBaptism ? 'Baptism' : 'Memorial Service';
  ...
  const detailLine = isWedding && coupleNames
    ...
    : isBaptism && baptismPersonName
      ...
      : (!isWedding && !isBaptism && deceasedName ? `<p><strong>In Memory of:</strong> ${deceasedName}</p>` : '');
...
${!isWedding && !isBaptism && deceasedName ? `- In Memory of: ${deceasedName}` : ''}
- Fee: $${fee.toFixed(2)}
