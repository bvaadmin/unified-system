 ...
 * @param {string} data.formType - 'wedding' or 'memorial-funeral-service'
...
export async function sendChapelConfirmation(data) {
  ...
  const serviceLabel = isWedding ? 'Wedding' : 'Memorial Service';
  ...
  const detailLine = isWedding && coupleNames
    ...
    : (!isWedding && deceasedName ? `<p><strong>In Memory of:</strong> ${deceasedName}</p>` : '');
...
${!isWedding && deceasedName ? `- In Memory of: ${deceasedName}` : ''}
- Fee: $${fee.toFixed(2)}
