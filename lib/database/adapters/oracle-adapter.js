...
function writeWalletFilesFromEnv() {
  try {
...
// Write wallet files on module load
const dynamicTnsAdminPath = writeWalletFilesFromEnv();
...
const oracleConfig = {
  ...
  tnsAdmin: dynamicTnsAdminPath || process.env.ORACLE_TNS_ADMIN || process.env.ORACLE_NETWORK_CONFIG_DIR,
...
function deriveConnectStringInfo() {
  ...
  const aliases = parseTnsAliases(oracleConfig.tnsAdmin || oracleConfig.walletLocation);
  if (aliases.length) {
...
export function getResolvedConnectInfo() {
  ...
  return {
    ...
    source: info.source,
    tnsAdmin: oracleConfig.tnsAdmin || null
  };
}
    ...
    if (!pool) {
      ...
      if (usingMTLS) {
        ...
        poolConfig = {
          ...
          walletLocation: oracleConfig.walletLocation || dynamicTnsAdminPath,
        ...
        poolConfig = {
          ...
          poolIncrement: oracleConfig.poolIncrement,
          poolTimeout: oracleConfig.poolTimeout,
          configDir: oracleConfig.tnsAdmin
        };
      } else {
