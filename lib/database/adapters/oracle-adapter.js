...
export async function initializePool() {
  try {
    if (!pool) {
      ...
      const dynamicTnsAdminPath = await writeWalletFilesFromEnv();
      ...
      if (dynamicTnsAdminPath) {
        console.log(`Initializing Oracle client with configDir: ${dynamicTnsAdminPath}`);
        oracledb.initOracleClient({ configDir: dynamicTnsAdminPath });
      }
      ...
      try {
        ...
        console.error('Error in createPool:', e);
        throw e;
      }
    }
    return pool;
