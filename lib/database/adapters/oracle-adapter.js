...
const oracleConfig = {
  ...
  // TNS Admin directory for one-way TLS (or general network config)
  ...
  try {
    if (!pool) {
      ...
      if (usingMTLS) {
        ...
        // Use TNS admin directory containing sqlnet.ora/tnsnames.ora (e.g., ./oracle_wallet)
        // The sqlnet.ora should include SSL_SERVER_DN_MATCH and any required SSL settings.
        poolConfig = {
          user: oracleConfig.user,
          password: oracleConfig.password,
          connectString: deriveConnectString(),
          ...
          poolIncrement: oracleConfig.poolIncrement,
          poolTimeout: oracleConfig.poolTimeout,
          configDir: oracleConfig.tnsAdmin || oracleConfig.walletLocation
        };
      } else {
